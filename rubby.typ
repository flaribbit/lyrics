...
#let _ruby(rt, rb, size, pos, dy, alignment, delimiter, auto-spacing) = {
  ...
  let add-spacing-if-enabled(text) = {
    ...
    return (
      if text.first() != delimiter { delimiter } + text + if text.last() != delimiter { delimiter }
    )
  }
...
#let get-ruby(
  ...
  _ruby(rb, rt, size, pos, dy, alignment, delimiter, auto-spacing)
)

#let ruby = get-ruby(auto-spacing: false)
#let bigskip() = v(1em)
#let zhfont = "Source Han Sans SC"
#let zhtext(body) = text(body, font: zhfont)
